...
const { raunyfb } = require('./arifirazzaq/bash/raunyfb')
...
const vcard = 'BEGIN:VCARD\n' //
            ...
            + 'ORG:Creator EREN;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=5514998360726:+5514998360726\n' //
            + 'END:VCARD'
prefix = '/'
blocked = []
       ...
       //function leveling
            ...
            break
				   ...
				   if (!isUser) return reply(mess.only.userB)
                    ...
                    if (isBanned) return reply(mess.only.benned)    
                    if (!isUser) return reply(mess.only.userB)
                    client.sendMessage(from, raunyfb(prefix), text, { quoted: mek })
                    tod = await getBuffer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
					case 'animesaran':
